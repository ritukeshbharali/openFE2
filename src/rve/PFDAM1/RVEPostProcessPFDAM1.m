%-------------------------------------------------------------------------%
% [RVESols,MacroGPdata] = RVEPostProcessPFDamage1(idx,step,dir_output)
...
function [RVESols,MacroGPdata] = RVEPostProcessPFDAM1(idx,step)
...
% Image names
RVEfigpf  = sprintf('Pf_Step%d_RVE%d', step , idx);
...
try   
    ...
    mphplot(model,'pg1','rangenum',2);
    saveas(f4,fullfile(dir_postproc,RVEfigpf),'png');
